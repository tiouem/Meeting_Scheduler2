...
{
    public class AddMeeting_VM : ObservableObject
    {
        ...
        {
            Database.DeleteMeeting(_newMeeting);
            var nav = new NavigationService();

            var md = new MessageDialog("Meeting deleted.");
            md.ShowAsyncQueue();
        ...
        private void _addMeetingClick()
        {
            if (EndTime.Subtract(StartTime).TotalMinutes > 0)
            {
                ...
                Database.AddMeeting(_newMeeting);                
            }
            else
            {
                var md = new MessageDialog("Meeting can't start at " + StartTime + " and end at " + EndTime+".");
                md.ShowAsyncQueue();
            }
        }
